var mongoose = require('mongoose')
  ...
  , MarketsHistory = require('../models/marketsHistory')
  ...
  , bitsane = require('./markets/bitsane')
  , newCapital = require('./markets/newCapital')
...
function find_tx_blockindex(blockindex, cb) {
...
function get_market_data(market, cb) {
  switch(market) {
    case 'bittrex':
      ...
      break;
    case 'bitsane':
        bitsane.get_data(settings.markets.coin, settings.markets.exchange, function(err, obj){
            return cb(err, obj);
        });
        break;
    case 'newCapital':
      newCapital.get_data(settings.markets.coin, settings.markets.exchange, function(err, obj){
          return cb(err, obj);
      });
      break;
    case 'bleutrade':
...
function get_market_trades(market, since, limit, cb) {
    switch(market) {
        case 'bitsane':
            bitsane.get_all_trades(settings.markets.coin, settings.markets.exchange, since, limit, function(err, obj){
                return cb(err, obj);
            });
            break;
        case 'newCapital':
            newCapital.get_all_trades(settings.markets.coin, settings.markets.exchange, since, limit, function(err, obj){
                return cb(err, obj);
            });
            break;
        default:
...
function get_market_latest_data(market, cb) {
    switch(market) {
        case 'bitsane':
            bitsane.get_latest_data(settings.markets.coin, settings.markets.exchange, function(err, obj){
                return cb(err, obj);
            });
            break;
        case 'newCapital':
            newCapital.get_latest_data(settings.markets.coin, settings.markets.exchange, function(err, obj){
                return cb(err, obj);
            });
            break;
        default:
...
module.exports = {
  ...
  get_walletscount: function(cb) {
    Address.count(function(err, count) {
      return cb(count);
    });
  },

  get_active_wallets_count: function(cb) {
    Address.find({'balance': {$gt: 0}}).count(function(err, count) {
      return cb(count);
    });
  },
  ...
  get_tx_blockindex: function(blockindex, cb) {
    find_tx_blockindex(blockindex, function(tx){
      return cb(tx);
    });
  },
  ...
  get_masternode_rewards: function(since, mnPayees, cb) {
      Tx.aggregate([
          ...
          if (err) {
            ...
            var total = 0;
            for(var i in data) {
              total += data[i].vout.amount;
            }
            data.push({total: total / 100000000})
              return cb(data);
          }
      });
  },

  get_masternode_rewards_total: function(since, mnPayees, cb) {
      Tx.aggregate([
          { $match: {
              "timestamp": { $gt: Number(since) },
              // "vin.addresses": "coinbase"
  ...
  create_market_history: function(coin, exchange, market, cb) {
      var newMarketsHistory = new MarketsHistory({
          market: market,
          coin: coin,
          exchange: exchange,
      });

      newMarketsHistory.save(function(err) {
          if (err) {
              console.log(err);
              return cb();
          } else {
              console.log("initial markets history entry created for %s", market);
              //console.log(newMarkets);
              return cb();
          }
      });
  ...
  check_market_history: function(market, cb) {
      MarketsHistory.findOne({market: market}, function(err, exists) {
          if(exists) {
              return cb(market, true);
          } else {
              return cb(market, false);
  ...
  // gets market history data for given market
  get_market_history: function(market, from, to, cb) {
      MarketsHistory.findOne({market: market,}, function(err, data) {
          if(data) {
              data.buys = data.buys.filter(function(obj) {
                return obj.timestamp >= from && obj.timestamp < to
              })
              data.sells = data.sells.filter(function(obj) {
                  return obj.timestamp >= from && obj.timestamp < to
              })
              return cb(data);
          } else {
  ...
  update_markets_db: function(market, cb) {
    ...
    update_markets_history_db: function(market, current_data, cb) {
        get_market_latest_data(market, function (err, obj) {
            if (err == null) {
                current_data.buys.sort(function(a, b) { // desc
                    return b.timestamp - a.timestamp;
                })
                current_data.sells.sort(function(a, b) { // desc
                    return b.timestamp - a.timestamp;
                })
                var currentBuys = current_data.buys;
                var currentSells = current_data.sells;
                obj.buys.sort(function(a, b) { // desc
                    return b.timestamp - a.timestamp;
                })
                obj.sells.sort(function(a, b) { // desc
                    return b.timestamp - a.timestamp;
                })

                //checking duplicated buys
                var new_buys = [];
                var currentBuysString = JSON.stringify(currentBuys);
                for(var i = 0; i < obj.buys.length; i++) {
                  if(currentBuysString.indexOf(JSON.stringify(obj.buys[i])) === -1) {
                      new_buys.push(obj.buys[i]);
                  }
                }
                currentBuys = new_buys.concat(currentBuys);
                // for(var i = 0; i < new_buys.length; i++) {
                //     currentBuys.push(new_buys[i])
                // }
                console.log('obj.buys.length', obj.buys.length);
                console.log('new_buys.length', new_buys.length);

                //checking duplicated sells
                var new_sells = [];
                var currentSellsString = JSON.stringify(currentSells);
                for(var i = 0; i < obj.sells.length; i++) {
                  if(currentSellsString.indexOf(JSON.stringify(obj.sells[i])) === -1) {
                      new_sells.push(obj.sells[i]);
                  }
                }
                currentSells = new_sells.concat(currentSells);
                // for(var i = 0; i < new_sells.length; i++) {
                //     currentSells.push(new_sells[i])
                // }
                console.log('obj.sells.length', obj.sells.length);
                console.log('new_sells.length', new_sells.length);
                console.log("currentBuys.length",currentBuys.length);
                console.log("currentSells.length",currentSells.length);
                // console.log("currentSells",currentSells);
                MarketsHistory.update({market:market}, {
                    buys: currentBuys,
                    sells: currentSells,
                    // buys: [],
                    // sells: [],
    ...
    get_market_trades: function(market,since,limit, cb) {
        get_market_trades(market, since, limit, function (err, obj) {
            if (err !== null) {
                return cb(err);
            }

            return cb(obj.trades);
        })
    },
    get_market_latest_data: function(market, cb) {
        get_market_latest_data(market, function (err, obj) {
            if (err !== null) {
                return cb(err);
            }
  ...
  update_tx_db: function(coin, start, end, timeout, cb) {
    var complete = false;
    console.log("start",start); // 7003
    console.log("end",end); // 7004
      ...
      lib.get_blockhash(start + x, function(blockhash){
        if (blockhash) {
          lib.get_block(blockhash, function(block) {
            if (block && block.tx) {
              console.log('block.tx.length', block.tx.length);
  ...
  update_address_db_only: function(coin, start, end, timeout, cb) {
    var complete = false;
    lib.syncLoop((end - start) + 1, function (loop) {
        var x = loop.iteration();
        lib.get_blockhash(start + x, function(blockhash){
